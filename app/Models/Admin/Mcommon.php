...
class Mcommon extends Model
{
 ...
 public function getdashboardrecords(){
    ...
    $todayorders = DB::table('orders')->where('status', "1")->where('order_status','NEW')->whereDate('created_date',$date)->count();
    $todayordersvalue = DB::table('orders')->where('status', "1")->where('order_status','NEW')->whereDate('created_date',$date)->sum('net_amount');
    $currentmonthorder = DB::table('orders')->where('status', "1")->where('order_status','NEW')->whereYear('created_date', Carbon::now()->year)->whereMonth('created_date', Carbon::now()->month)->sum('net_amount');
    $todayorderslist = DB::table('orders')->where('status',"1")->where('order_status','NEW')->whereDate('created_date', $date)->orderBy('id', 'DESC')->limit(10)->get()->toArray();
    $todaynewuser = DB::table('users')->where('status',"1")->whereDate('created_date', $date)->count();
    ...
    "todayordervalue"=>$todayordersvalue,
    "todaynewuser"=>$todaynewuser,
    "currentmonthordervalue"=>$currentmonthorder,
