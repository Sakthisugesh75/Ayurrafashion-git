...
if (!empty($items->size)) {
    ...
    foreach ($sizes as $sizeId) {
        ...
        if ($size) {
            $sizeDetails[] = strtoupper(trim($size)); // Normalize (S, M, L, XL)
        }
    }

    // Custom order
    $order = ["XS", "S", "M", "L", "XL", "XXL", "3XL", "4XL", "5XL"];

    // Sort based on custom order
    usort($sizeDetails, function($a, $b) use ($order) {
        $posA = array_search($a, $order);
        $posB = array_search($b, $order);

        // If size not in array, push to end
    ...
    // Assign sorted size names
    $items->size_name = implode(',', $sizeDetails);

} else {
    $items->size_name = null;
}
