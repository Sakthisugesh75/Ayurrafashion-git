...
(function ($) {
  ...
  var totalPriceVariant = function () {
    ...
    $(".color-btn, .size-btn").on("click", function () {
      ...
      quantityInput.val(1);
      $(".price-on-sale").text("Rs." + newPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
      var totalPrice = newPrice;
      $(".total-price").text("Rs." + totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ","));
    });
    ...
    function updateTotalPrice() {
      var currentPrice = parseFloat($(".price-on-sale").text().replace("Rs.", ""));
